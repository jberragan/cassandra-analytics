...
public class SparkSqlTypeConverterImplementation implements SparkSqlTypeConverter
{
    ...
    static
    {
        ...
        types.put(org.apache.cassandra.spark.data.types.Boolean.class, org.apache.cassandra.spark.data.converter.types.Boolean.INSTANCE);
    ...
    public static SparkType toSparkType(CqlField.CqlType cqlType)
    {
        return getOrThrow(cqlType);
    }

    public Object convert(CqlField.CqlType cqlType, @NotNull Object value)
    {
        return getOrThrow(cqlType).toSparkSqlType(value, false); //FIXME: isFrozen
    }
    ...
    protected static SparkType getOrThrow(CqlField.CqlType cqlType)
    {
        ...
        if (cqlType.isFrozen())
        {
            ...
            return getOrThrow(((CqlField.CqlFrozen) cqlType).inner());
        }
        ...
        if (cqlType.isComplex())
        {
            ...
            {
                return new org.apache.cassandra.spark.data.converter.types.complex.Set((CqlField.CqlSet) cqlType);
            }
            else if (cqlType instanceof CqlField.CqlList)
            {
                return new List((CqlField.CqlList) cqlType);
            }
            else if (cqlType instanceof CqlField.CqlMap)
            {
                return new org.apache.cassandra.spark.data.converter.types.complex.Map((CqlField.CqlMap) cqlType);
            }
            else if (cqlType instanceof CqlField.CqlTuple)
            {
                return new org.apache.cassandra.spark.data.converter.types.complex.Tuple((CqlField.CqlTuple) cqlType);
            }
            else if (cqlType instanceof CqlField.CqlUdt)
            {
                return new Udt((CqlField.CqlUdt) cqlType);
            }
